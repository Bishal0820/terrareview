...
def format_markdown(
    ...
    # Footer
    lines.append("---")
    footer = "*Generated by [tfrev](https://github.com/bishalOps/tfrev)"
    if api_response is not None:
        footer += f" · {api_response.input_tokens:,} tokens in / {api_response.output_tokens:,} out"
        if duration_s is not None:
            footer += f" · {duration_s:.1f}s"
        footer += f" · {api_response.model} · {config.provider}"
    footer += "*"
    lines.append(footer)
...
def format_table(
    ...
    if not findings:
        ...
        _append_table_footer(lines, api_response, duration_s, config.provider)
        return "\n".join(lines)
...
def _append_table_footer(
    ...
    if api_response is None:
        return
    parts = [
        f"{api_response.input_tokens:,} tokens in / {api_response.output_tokens:,} out",
    ]
    if duration_s is not None:
        parts.append(f"{duration_s:.1f}s")
    parts.append(api_response.model)
    parts.append(provider)
    lines.append(click.style("  " + " · ".join(parts), dim=True))
